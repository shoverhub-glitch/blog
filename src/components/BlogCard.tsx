...
export const BlogCard = ({ blog, variant = 'default' }: BlogCardProps) => {
  ...
  if (variant === 'featured') {
    return (
      <>
        <style>{`
          #${cardId} { transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); }
          #${cardId}:hover { transform: translateY(-6px); }
          #${cardId}:hover .card-img { transform: scale(1.06); }
          #${cardId}:hover .card-arrow { opacity: 1; transform: translate(0, 0); }
          #${cardId}:hover .card-overlay { opacity: 1; }
          .card-img { transition: transform 0.6s cubic-bezier(0.16, 1, 0.3, 1); }
          .card-arrow { transition: all 0.3s ease; opacity: 0; transform: translate(-4px, 4px); }
  ...
  if (variant === 'horizontal') {
    return (
      <>
        <style>{`
          #${cardId}-h { transition: all 0.3s ease; }
          #${cardId}-h:hover { background: ${theme.colors.surfaceHover}; }
          #${cardId}-h:hover .h-img { transform: scale(1.05); }
          .h-img { transition: transform 0.4s ease; }
        `}</style>
  ...
  return (
    <>
      <style>{`
        #${cardId}-d { transition: all 0.35s cubic-bezier(0.16, 1, 0.3, 1); }
        #${cardId}-d:hover { transform: translateY(-4px); box-shadow: ${theme.shadows.lg}; border-color: ${theme.colors.accent}20 !important; }
        #${cardId}-d:hover .d-img { transform: scale(1.04); }
        #${cardId}-d:hover .d-title { color: ${theme.colors.accent}; }
        .d-img { transition: transform 0.5s cubic-bezier(0.16, 1, 0.3, 1); }
        .d-title { transition: color 0.2s; }
      `}</style>
      <Link to={`/blog/${blog.slug}`} onMouseEnter={() => prefetchRoute('blog-detail')} onFocus={() => prefetchRoute('blog-detail')} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
        <article id={`${cardId}-d`} style={{
          background: theme.colors.surface,
          borderRadius: '8px',
          overflow: 'hidden',
          border: `1px solid ${theme.colors.border}`,
